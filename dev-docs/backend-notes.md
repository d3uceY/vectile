...
Folder exclusions. The project walker prunes directories by name (`project_exclude_folders`, `node_modules` by default). Without it a repo that vendors its dependencies ends up with thousands of package READMEs and changelogs in its collection, and they outrank the project's own docs in search: an early index of this repo held 1,944 `node_modules` markdown files against 16 of its own. Names match a whole directory component at any depth (so every nested `node_modules` is pruned), hidden directories were already skipped by the existing dot check, and a path the user configured explicitly is always walked so the list can never silently empty a collection. Obsidian vaults keep their own separate list (`obsidian_exclude_folders`), since those hold notes, not dependencies.
